...
(function () {
  ...
  /* ---------- build DOM ---------- */
  var ASSISTANT_IMG = "/assets/images/android-chrome-192.png";

  var fab = document.createElement("button");
  fab.className = "wa-fab";
  fab.setAttribute("aria-label", "Chat with Easynet on WhatsApp");
  fab.innerHTML =
    '<span class="wa-fab-avatar" aria-hidden="true">' +
      '<img src="' + ASSISTANT_IMG + '" alt="" width="38" height="38">' +
      '<i class="wa-fab-status"></i>' +
    '</span>' +
    '<span class="wa-fab-label">Chat with Easynet</span>';
  ...
  panel.innerHTML =
    '<div class="wa-head">' +
      '<div class="wa-avatar"><img src="' + ASSISTANT_IMG + '" alt="Easynet Assistant logo" width="36" height="36"></div>' +
      '<div class="wa-head-info"><b>Easynet Assistant</b><span><i class="wa-dot"></i>Online · replies instantly</span></div>' +
  ...
  /* ---------- greeting teaser — assistant pops up 7s after the visit ---------- */
  var teaser = document.createElement("div");
  teaser.className = "wa-teaser";
  teaser.setAttribute("role", "status");
  teaser.innerHTML =
    '<button class="wa-teaser-close" aria-label="Dismiss message">✕</button>' +
    '<div class="wa-teaser-head">' +
      '<img src="' + ASSISTANT_IMG + '" alt="">' +
      '<b>Easynet Assistant</b><span>Online</span>' +
    '</div>' +
    '<div class="wa-teaser-text"><span class="wa-typing"><i></i><i></i><i></i></span></div>';

  var teaserTimer = null;        // the 7s delay before the bubble appears
  var teaserTypingTimer = null;  // "typing…" → greeting text
  var teaserAutoTimer = null;    // auto-dismiss so it never blocks the page
  var teaserShown = false;

  function dismissTeaser() {
    if (teaserTimer) { clearTimeout(teaserTimer); teaserTimer = null; }
    if (teaserTypingTimer) { clearTimeout(teaserTypingTimer); teaserTypingTimer = null; }
    if (teaserAutoTimer) { clearTimeout(teaserAutoTimer); teaserAutoTimer = null; }
    fab.classList.remove("wa-nudge");
  ...
  function showTeaser() {
    if (teaserShown || panel.classList.contains("open")) return;
    teaserShown = true;
    document.body.appendChild(teaser);
    requestAnimationFrame(function () {   // let the entrance transition play
      requestAnimationFrame(function () { teaser.classList.add("show"); });
    });
    teaserTypingTimer = setTimeout(function () {
      teaser.querySelector(".wa-teaser-text").textContent = "Hi \uD83D\uDC4B How may I help you?";
      fab.classList.add("wa-nudge");
    }, 1400);
    teaserAutoTimer = setTimeout(dismissTeaser, 34000);
  }

  teaser.addEventListener("click", function (e) {
    var t = e.target;
    if (t && t.classList && t.classList.contains("wa-teaser-close")) { dismissTeaser(); return; }
    dismissTeaser();
    openPanel();
  });
  fab.addEventListener("animationend", function () { fab.classList.remove("wa-nudge"); });

  teaserTimer = setTimeout(showTeaser, 7000);   /* ← the 7-second popup */
